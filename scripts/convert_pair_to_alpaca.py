...
def create_prompt(slines, tlines, ins_list, max_cxt, fix_cxt, no_rand_breaker):
    prompts = []
    for i in range(len(slines)):
        p = dict()
        num_demo = random.randint(0, max_cxt) if fix_cxt < 0 else fix_cxt
        st = max(0, i - num_demo)
...
def write_json(src_file, tgt_file, out_file, ins_list, max_cxt, seed=0, fix_cxt=-1, no_rand_breaker=False):
    random.seed(seed)
    np.random.seed(seed)
    with open(src_file, 'r', encoding='utf-8') as fs, open(tgt_file, 'r', encoding='utf-8') as ft, \
        ...
        prompts = create_prompt(slines, tlines, ins_list, max_cxt, fix_cxt, no_rand_breaker)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--fix-cxt', type=int, default=-1, help='fixed max context')
    parser.add_argument('--seed', type=int, default=0, help='random seed')
    parser.add_argument('--no_rand_breaker', action='store_true', default=False, help='no rand linebeaker')    
    ...
    fix_cxt = args.fix_cxt
    seed = args.seed
    no_rand_breaker = args.no_rand_breaker

    # Start
    ins_list = read_instruct(ins_file, src, tgt, lang_ins)
    print("Number of instructs: {}".format(len(ins_list)))
    write_json(src_file, tgt_file, out_file, ins_list, max_cxt, seed, fix_cxt, no_rand_breaker)
